...
- (id)init
{
    ...
    if (self) {
		...
		NSMutableSet * connections = [[[NSMutableSet alloc]init]autorelease];
...
-(IBAction)renderGraphics:(id)sender{
    ...
    int c = [[self materials]count]*kNodeSize*.6;
    int width = c > 4000 ? c : 4000;
    int height = width * (1.0/pow(2, 0.5));//0.70692046;
    [graphCreator resizeTo:NSMakeSize(width, height)];
