...
#define kResizeFactor 1.3
#define kRepositionDepthMax 200
...
-(void)renderIdentityNodes{
	...
	NSLog(@"found %lu identities", [identityNames count]);
...
-(void)fixIdentityDistances{
	...
	NSArray * sortedIdentities = [[self sortedIdentities]retain];
	...
	[doc displayProgress:YES];
   	for(i=0;i<[sortedIdentities count];i++){
        NSLog(@"fixing identityNodeLocation: %d", i);
		n = [sortedIdentities objectAtIndex:i];	// get current node;
        NSArray * ellipse = [self pointArrayForEllipse];
        		
        points = [[self intersectionPointsOnEllipse:ellipse forNode:n withAverageDistance:avgDistance]retain];	//get it's intersectionpoints
		//NSLog(@"points for node %@: %@", [n name], points);
        NSLog(@"sortedIdentities: %lu", [sortedIdentities count]);
...
-(NSArray *)removeDuplicatePointsFromArray:(NSArray *)points withTolerance:(double)d{
	...
	//double xa,xb, ya, yb;
	//	NSPoint a, b;
	NSMutableArray * new = [[[NSMutableArray alloc]init]autorelease];
...
//		[doc setProgress:(100.0/[[doc materials] count]) * ([[doc materials] count] - [materialNames count])];
		...
		perfectLocation = [self perfectLocationForMaterial:mat];//[self perfectLocationForMaterialNodeWithName:name];	
		n = [[[Node alloc]init]autorelease];
	...
	[doc setIndeterminateProgressTask:@"rendering material node paths..."];
	...
	[doc setIndeterminateProgressTask:@"rendering connections..."];
    ...
    [doc setIndeterminateProgressTask:@"statistics..."];
	[self createDateString];
    NSLog(@"date string successfully");
    [doc setIndeterminateProgressTask:@"almost there..."];
	//[graph setNeedsDisplay:YES];
...
-(NSPoint)perfectLocationForMaterialNodeWithName:(NSString *)s{
    ...
    return [self perfectLocationForMaterial:mat];
}

-(NSPoint)perfectLocationForMaterial:(Material *)mat{
//	Material * mat = [doc getMaterialByName:s];
	NSPoint center = [self center];
	int i;
...
-(BOOL)repositionNodeIfNecessary:(Node*)c{
	//[doc setIndeterminateProgressTask:@"repositioning..."];
	Node * n; 
	//repositionDepth=0;
	while((n = [self collidingNodeForNode:c]) && repositionDepth < kRepositionDepthMax){
		...
		if (![self isPoint:[c location] inRect:[self bounds]] || ![self isPoint:[c center] inRect:[self bounds]] ||
			...
			//NSLog(@"out of bounds!");
			NSPoint perfect = [self perfectLocationForMaterial:[c material]];//[self perfectLocationForMaterialNodeWithName:[c name]];
...
-(void) repositionNodes: (Node *) nodeA and: (Node*) nodeB{ 
    ...
    if([self isNodeOutsideIdentityRim:na]){
        if([[na pullCount]intValue] < kRepositionDepthMax)
            [self repositionNodeInsideIdentityRim:na];
    }
    
    if([self isNodeOutsideIdentityRim:nb]){
        if([[nb pullCount]intValue] < kRepositionDepthMax)
...
-(void)repositionNodeInsideIdentityRim:(Node *)n{
    ...
    for(int i = 0;[self isNodeOutsideIdentityRim:n];i++){
        if(i>7){
            ...
            [n setLocation:[self perfectLocationForMaterial:[n material]]];//NodeWithName:[n name]]];//[self pullNodeTowardCenter:na];
            break;
        }
...
-(void)pullNodeTowardCenter:(Node *)n{
    ...
    [n setPullCount:[NSNumber numberWithInt:[[n pullCount]intValue]+1]];
...
-(void)createPathsForConnections{
	...
	int i, o;
	float width, sizeWidth = [self size].width, offset, maxWidth = 0;// =120;
    ...
    [self setValue:[NSNumber numberWithFloat:maxWidth] forKey:@"maximumPathWidth"];
    
    
    
    NSMutableArray * allPaths = [[[NSMutableArray alloc] initWithArray:[[self valueForKey:@"connectionPaths"]allObjects]]autorelease];
    
     NSArray * sortedPaths = [allPaths sortedArrayUsingComparator: ^(id obj1, id obj2) {
        
        if ([obj1 lineWidth] > [obj2 lineWidth]) {
    ...
    [self setValue:sortedPaths forKey:@"sortedConnectionPaths"];
	//NSLog(@"%@", [self valueForKey:@"sortedConnectionPaths"]);
}
