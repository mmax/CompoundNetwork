...
//	NSLog(@"graph material nodes: %d", [materialNodes count]);
    ...
    NSArray * connectionPaths = [controller valueForKey:@"sortedConnectionPaths"];
    ...
    maxWidth = [[controller valueForKey:@"maximumPathWidth"]floatValue];
...
//            c=0;
//        else
            //c = 1.0 - pow(1.0 - (0.7 - (c/maxWidth)*.7), 2); //scale to .7, offset to 0.3 -> 1.0, inverse, square, inverse
        c = .2 - pow(c/maxWidth, 3)*.2;
        
        //NSLog(@"width: %f, c: %f, max: %f", [path lineWidth], c, maxWidth);
        [[NSColor colorWithDeviceRed:c green:c blue:c alpha:1]set];		
